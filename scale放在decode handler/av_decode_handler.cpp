...
void AVDecodeHandler::Loop()
{
	...
	while (!is_exit_)
	{
		...
		if (next && is_stream_copy_ && !is_need_scale_)
		{
...
void AVDecodeHandler::SetStreamCopy(bool status)
{
	unique_lock<mutex> lock(mtx_);
	is_stream_copy_ = status;
}

int AVDecodeHandler::CopyVCodecExtraData(uint8_t* buffer, int& size)
{
	return decoder_.GetCodecExtraData(buffer, size);
}

void AVDecodeHandler::SetNeedScale(bool status)
{
	unique_lock<mutex> lock(mtx_);
	is_need_scale_ = status;
}

void AVDecodeHandler::SetNeedResample(bool status)
{
	unique_lock<mutex> lock(mtx_);
	is_need_resample_ = status;
}

void AVDecodeHandler::SetScaler(int width, int height, int fmt)
{
	unique_lock<mutex> lock(mtx_);
	scaler_width_ = width;
